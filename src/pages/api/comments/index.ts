...
export const GET: APIRoute = async ({ url, cookies }) => {
  ...
  if (!isCommentTargetType(targetType)) return badRequest('目标类型不合法');
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  if (!isCommentTargetType(body.targetType)) return badRequest('目标类型不合法');
  const targetId = typeof body.targetId === 'string' ? body.targetId.trim() : '';
  if (!targetId || targetId.length > 200) return badRequest('目标 ID 不合法');
  ...
  if (githubUserId) {
    ...
    if (!authorName) return badRequest('匿名评论请填写昵称');
  }
