...
### 📚 归档（`/archive`）
- 时间线式归档：按「年 → 月」倒序分组，左侧连续竖轴 + 节点圆点
- 卡片含日号/星期、分类徽章、字数、阅读量、摘要，封面可选（有则显示右侧缩略图，无则整块省略）
- 右栏为**标签 + 分类筛选面板**：点击即当前页内即时过滤（无刷新），被筛空的年/月分组自动隐藏；标签与分类互斥，再点一次取消。窄屏折叠为 `<details>` 保证可达
